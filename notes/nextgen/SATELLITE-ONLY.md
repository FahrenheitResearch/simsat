...
Active scope is overhead simulated satellite imagery and validation. The user clarified that the primary purpose is helping meteorologists during cloud-seeding missions. The user confirmed both glaciogenic and hygroscopic seeding, and all three workflows: planning, interpretation during missions, and post-mission review. Meteorological interpretability and fidelity therefore take priority over cosmetic realism. Native cloud-water/ice maps are model diagnostics, not observations or seeding-suitability assessments. The 1974 severe-storm case is a rendering stress case; it is not a mission validation case. Ground and free-perspective work is set aside. Worktree: C:/Users/drew/simsat-wt-nextgen-abi; branch codex/nextgen-abi. Main is unchanged, nothing merged or pushed, Cargo jobs remains 6.
...
## Mission requirements clarified by the user

The primary audience is meteorologists supporting both glaciogenic and hygroscopic cloud-seeding missions. They need planning, in-mission interpretation, and post-mission review. The product remains overhead simulated satellite imagery. This does not reopen ground-camera or GUI development.

The imagery should be accompanied by explicitly labeled model quantities (liquid/ice phase, water content/path, temperature/height, and vertical structure) and collocated observations. Preserve original species and units. A cloud's rendered appearance must not be used to infer a seeding opportunity or a seeded-versus-unseeded response. No seeding treatment operator or mission effectiveness validation has been implemented.

True-color C01/C02/C03 alone is not the whole mission product. NOAA's [Day Cloud Phase Distinction guide](https://www.star.nesdis.noaa.gov/goes/documents/ABIQuickGuide_DayNightCloudMicroCombo.pdf) uses C13 (10.3 um), C02 (0.64 um), and C05 (1.6 um) to help distinguish cloud-top phase. That is a relevant future spectral target; it cannot be synthesized honestly by coloring the native ice/liquid mass maps or reusing existing RGB. Cloud-top remote sensing must be interpreted alongside the model's vertical fields, not presented as direct observation of all water below cloud top.
